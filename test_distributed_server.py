...
from distributed_server import (
    DistributedTaskManager, Task, Agent, AgentType,
    TaskStatus, TaskPriority, task_manager
)
from enhanced_distributed_server import (
    coordinate_project, deploy_application
)
...
class TestDistributedTaskManager:
    ...
    def test_create_task(self, clean_task_manager):
        """Test task creation"""
        task = clean_task_manager.create_task(
            "Test Task",
            "Test Description",
            "frontend",
            TaskPriority.HIGH
...
class TestIntegrations:
    ...
    async def test_git_create_pr(self, mock_git, mock_context):
        ...
        mock_git.create_pr = AsyncMock(return_value={
            "number": 42,
    ...
    async def test_jenkins_build(self, mock_jenkins, mock_context):
        ...
        mock_jenkins.trigger_job = AsyncMock(return_value={
            "status": 201,
            "location": "https://jenkins/queue/item/123"
        })
        
        result = await jenkins_build(
            mock_context,
            "test-job",
            {"BRANCH": "main"}
        )
    ...
    async def test_aws_operations(self, mock_aws, mock_context):
        ...
        # Test EC2
        mock_aws.ec2_operation = AsyncMock(return_value={
        ...
        # Test S3
        mock_aws.s3_operation = AsyncMock(return_value={
            "status": "uploaded",
            "url": "s3://bucket/key"
        })
    ...
    async def test_bambu_print(self, mock_bambu, mock_context):
        ...
        from enhanced_distributed_server import bambu_send_print, bambu_printer_status
        
        # Test sending print
        mock_bambu.send_print_job = AsyncMock(return_value={
            "status": "queued",
            "job_id": "job_123",
            "estimated_time": "2h 30m"
        })
        
        result = await bambu_send_print(
            mock_context,
            "X1-Carbon-01",
            "/models/test.3mf",
            "PETG",
        ...
        # Test printer status
        mock_bambu.get_printer_status = AsyncMock(return_value={
...
class TestCoordination:
    ...
    @patch('enhanced_distributed_server.slack_integration')
    async def test_coordinate_project(self, mock_slack, mock_youtrack, 
                                     mock_context, clean_task_manager):
    ...
    @patch('enhanced_distributed_server.jenkins_integration')
    @patch('enhanced_distributed_server.slack_integration')
    async def test_deploy_application(self, mock_slack, mock_jenkins,
                                     mock_context, clean_task_manager):
        """Test application deployment"""
        mock_jenkins.trigger_job = AsyncMock(return_value={
            "status": 201,
            "location": "https://jenkins/queue/123"
        })
        mock_slack.send_message = AsyncMock(return_value={"ok": True})
