...
async def calculate(ctx: Context, expression: str) -> str:
    ...
    """
    try:
        # Use ast module for safe expression evaluation
        import ast
        import operator
...
async def get_config() -> str:
    ...
    config = {
        ...
        "limits": {"max_request_size": "10MB", "timeout": "30s"},
    }
...
async def greeting(name: str, formal: bool = False) -> List[PromptMessage]:
    ...
    return [PromptMessage(role="user", content=TextContent(type="text", text=template))]
...
@mcp.tool()
async def process_data(ctx: Context, data: List[Dict[str, Any]], operation: str, options: Dict[str, Any] = None) -> str:
    """
