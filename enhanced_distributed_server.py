...
from integrations import (
    slack_integration, git_integration, jenkins_integration,
    youtrack_integration, postgres_integration, aws_integration,
    bambu_integration, web_integration, api_integration
)
...
@mcp.tool()
async def slack_send(ctx: Context, 
                    channel: str, 
                    message: str,
...
@mcp.tool()
async def git_create_pr(ctx: Context,
                       repo: str,
                       title: str,
                       body: str,
                       head_branch: str,
...
@mcp.tool()
async def jenkins_build(ctx: Context,
                       job_name: str,
...
@mcp.tool()
async def youtrack_create(ctx: Context,
                         project: str,
                         summary: str,
                         description: str = "",
...
@mcp.tool()
async def postgres_execute(ctx: Context,
                          query: str,
...
@mcp.tool()
async def bambu_send_print(ctx: Context,
                          printer_id: str,
                          file_path: str,
                          material: str = "PLA",
...
@mcp.tool()
async def coordinate_project(ctx: Context,
                           project_name: str,
                           requirements: str,
    ...
    try:
        ...
        issue_result = await youtrack_integration.create_issue(
            integrations.youtrack.default_project,
            f"{project_name} - Main Project",
            requirements
        )
...
@mcp.tool()
async def deploy_application(ctx: Context,
                           app_name: str,
    ...
    try:
        ...
        await slack_integration.send_message(
            integrations.slack.default_channel,
            f"🚀 Deploying {app_name} to {environment} from branch {branch}"
        )
        ...
        devops_agents = [a for a in task_manager.agents.values() 
                        if a.type == AgentType.DEVOPS]
        if devops_agents:
