...
async def test_process_data_filter(mock_context):
    ...
    result = await process_data(mock_context, data, "filter", {"key": "age", "value": 30})
...
async def test_process_data_transform(mock_context):
    ...
    result = await process_data(mock_context, data, "transform", {"key": "name", "type": "uppercase"})
...
async def test_process_data_aggregate(mock_context):
    ...
    # Test count
    result = await process_data(mock_context, data, "aggregate", {"type": "count"})
    assert "Aggregation result: 3" in result

    # Test sum
    result = await process_data(mock_context, data, "aggregate", {"key": "score", "type": "sum"})
...
async def test_process_data_error_handling(mock_context):
    ...
    result = await process_data(mock_context, [{"test": 1}], "unknown_op")
    assert "Unknown operation" in result

    # Test with invalid data
    result = await process_data(mock_context, "not a list", "filter")  # Invalid data type
